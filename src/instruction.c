...
{
    int diff = (is_zero(emu) || (is_sign(emu) != is_overflow(emu))) ? 0 : get_sign_code8(emu, 1);
    emu->eip += (diff + 2);
}

static void jge (Emulator* emu)
{
    int diff = (is_sign(emu) != is_overflow(emu)) ? 0 : get_sign_code8(emu, 1);
    emu->eip += (diff + 2);
}
...
{
    uint32_t imm8 = get_sign_code8(emu, 1);
    push32(emu, imm8);
    emu->eip += 2;
}
